...
public class LoadedResources implements SettingChangedListener {
    ...
    public void playSound(int resourceId, float leftVolume, float rightVolume, int priority, int loop, float rate){
        ...
        if (soundEnabled){
            try{
                soundPoolId = (int)loadedSounds.get(resourceId);
                soundStatus = (int)soundsStatus.get(soundPoolId);
                if (soundStatus == 0){
                    soundPool.play(resourceId, leftVolume, rightVolume, priority, loop, rate);
                }
            }
            catch (Exception e){
